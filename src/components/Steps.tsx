...
const StepCard: React.FC<StepCardProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`relative flex h-28 w-28 items-center justify-center hand-drawn-border border-2 border-text-main shadow-sm transition-transform duration-300 hover:scale-105
      ...
      >
        {/* Number Badge */}
        <div
          className={`absolute -top-4 -right-2 flex h-8 w-8 items-center justify-center rounded-full font-display text-xl border-2 border-white
        ...
        {badge && (
          <div
            className={`absolute -bottom-3 px-2 py-0.5 text-[10px] font-bold border border-text-main rounded-full shadow-sm flex items-center gap-1
      ...
      <div className="flex flex-col gap-1">
        <h3
          className={`text-2xl font-bold font-display ${darkMode ? 'text-gray-100' : 'text-text-main'}`}
        >
          {title}
        </h3>
        <p
          className={`text-base font-medium ${darkMode ? 'text-gray-400' : 'text-text-secondary'}`}
        >
          {subtitle}
        </p>
      </div>
...
const Steps: React.FC<StepsProps> = ({ darkMode, t }) => {
  return (
    <div className="w-full mt-8 relative">
      ...
      <svg
        className="hidden lg:block absolute top-28 left-0 w-full h-16 -z-10 opacity-20 pointer-events-none"
        fill="none"
        viewBox="0 0 1200 100"
        preserveAspectRatio="none"
      >
        <path
          d="M50,50 C300,20 900,80 1150,50"
          stroke={darkMode ? '#ffffff' : '#4a4a4a'}
