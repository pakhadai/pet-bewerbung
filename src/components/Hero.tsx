...
const Hero: React.FC<HeroProps> = ({ darkMode, t, onStartClick }) => {
  const defaultFreeFeatures = [
    ...
    'KI-Textgenerierung',
    'PDF-Download',
    'ZIP mit allen Designs',
  ]
  const freeFeaturesRaw = t?.hero?.freeFeatures
  const freeFeatures = Array.isArray(freeFeaturesRaw)
    ? freeFeaturesRaw
    : typeof freeFeaturesRaw === 'string'
      ? [freeFeaturesRaw]
      : defaultFreeFeatures
  ...
  return (
    <div className="flex flex-col items-center gap-4 max-w-5xl">
      ...
      <h1
        className={`text-5xl sm:text-6xl lg:text-8xl font-bold font-display leading-none mt-1 transition-colors
      ...
      <div
        className={`w-full max-w-4xl my-4 p-8 rounded-2xl border-l-4 shadow-lg text-left transition-colors
      ...
      >
        <div className="flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="flex-1">
            <h2
              className={`font-display font-bold text-xl mb-3 flex items-center gap-2 ${darkMode ? 'text-red-300' : 'text-red-500'}`}
            >
            ...
            >
              {t?.hero?.problemDesc ||
                'In der Schweiz ist der Wohnungsmarkt hart umkämpft. Viele Vermieter sind skeptisch gegenüber Haustieren.'}
            </p>
          </div>
          {/* Divider (desktop only) */}
          <div
            className={`hidden md:block w-px h-24 self-center ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}
          ></div>
          {/* Right: Solution */}
          <div className="flex-1">
            <h2
              className={`font-display font-bold text-xl mb-3 flex items-center gap-2 ${darkMode ? 'text-green-400' : 'text-green-600'}`}
            >
            ...
            >
              {t?.hero?.solutionDesc ||
                'Ein professioneller Lebenslauf belegt Versicherung & Erziehung. Erhöhen Sie Ihre Chancen auf die Traumwohnung massiv.'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto mt-6 justify-center">
        ...
        >
          <span className="relative flex items-center justify-center gap-4">
            ...
            <MaterialIcon
              name="lock"
              className="text-3xl group-hover:translate-x-2 group-hover:scale-110 transition-all duration-500"
            />
          </span>
          {/* Shadow element */}
          <div
            className={`absolute -bottom-1.5 -right-1.5 w-full h-full -z-10 rounded-xl border-2 border-transparent transition-all duration-500 group-hover:-bottom-2 group-hover:-right-2 ${darkMode ? 'bg-white/10' : 'bg-primary/20'}`}
          ></div>
        </button>
      </div>
      ...
      {/* Features - All Free */}
      <div
        className={`mt-8 max-w-3xl w-full rounded-2xl overflow-hidden shadow-lg
        ${darkMode ? 'bg-gray-800/80 border border-gray-700' : 'bg-white border border-gray-200'}`}
      >
        <h2
          className={`font-display font-bold text-xl text-center py-4 ${darkMode ? 'text-white bg-gradient-to-r from-gray-700 to-gray-800' : 'text-gray-800 bg-gradient-to-r from-gray-50 to-gray-100'}`}
        >
          ...
          <ul className={`space-y-3 text-sm ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>
            {freeFeatures.map((feature: string, i: number) => (
              <li key={i} className="flex items-center gap-3">
                <MaterialIcon
                  name="check"
                  className={`text-base ${darkMode ? 'text-green-400' : 'text-green-500'}`}
                />
                {feature}
              </li>
            ))}
