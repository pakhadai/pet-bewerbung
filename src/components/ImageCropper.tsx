...
const ImageCropper: React.FC<ImageCropperProps> = ({
  ...
  const getCroppedImg = useCallback(async () => {
    ...
    const drawToCanvas = (source: HTMLImageElement | ImageBitmap) => {
      const scaleX =
        'naturalWidth' in source && source.naturalWidth ? source.naturalWidth / image.width : 1
      const scaleY =
        'naturalHeight' in source && source.naturalHeight ? source.naturalHeight / image.height : 1
