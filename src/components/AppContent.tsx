...
const AppContent: React.FC = () => {
  ...
  const generateText = () => {
      ...
      body.push(
        r(
          data.behaviorWithChildren === 'good'
            ? pack.social.childrenGood
            : pack.social.childrenNeutral
        )
      )
    }
    ...
    if (data.behaviorWithPets && data.behaviorWithPets !== 'avoid') {
      body.push(
        r(data.behaviorWithPets === 'good' ? pack.social.petsGood : pack.social.petsNeutral)
      )
    }
  ...
  const handleDownloadPDF = async (templateType: TemplateType = 'classic') => {
