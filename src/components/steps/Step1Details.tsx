...
import type { FormData, Gender, PetType } from '../../types/form'
...
const Step1Details: React.FC<Step1DetailsProps> = ({ embedded = false }) => {
  ...
  const isPetType = (v: string): v is PetType => v === 'dog' || v === 'cat' || v === 'other'
  const isGender = (v: string): v is Gender => v === 'm' || v === 'f'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              {touched.ownerName && errors?.ownerName && (
                <span className="text-red-500 text-xs mt-1 block">
                  {t?.validation?.ownerNameRequired ?? 'Name is required (min. 2 characters)'}
                </span>
              )}
            </div>
            <div>
              ...
              <FormInput
                value={data.street ?? ''}
                onChange={(val) => updateData('street', val)}
                placeholder={t?.placeholders?.street ?? 'e.g. Bahnhofstrasse'}
              />
            </div>
            <div>
              ...
              <FormInput
                ...
                onChange={(val) => updateData('houseNumber', val)}
                placeholder="12A"
              />
            </div>
            <div>
              ...
              {touched.postal && errors?.postal && (
                <span className="text-red-500 text-xs mt-1 block">
                  {t?.validation?.postalInvalid ?? 'Invalid'}
                </span>
              )}
            </div>
            <div>
            ...
            <div>
              ...
              {touched.email && errors?.email && (
                <span className="text-red-500 text-xs mt-1 block">
                  {t?.validation?.emailInvalid ?? 'Invalid'}
                </span>
              )}
            </div>
            <div>
              ...
              {touched.phone && errors?.phone && (
                <span className="text-red-500 text-xs mt-1 block">
                  {t?.validation?.phoneInvalid ?? 'Invalid'}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="space-y-4 pt-6 border-t-2 border-dashed border-gray-400/50 dark:border-gray-500/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <div
                className={`grid grid-cols-3 gap-3 ${touched.petType && errors?.petType ? 'ring-2 ring-red-500 rounded-xl p-1' : ''}`}
              >
                {petTypes.map((type) => (
                  <button
                    key={type.id}
                    type="button"
                    onClick={() => {
                      if (isPetType(type.id)) updateData('petType', type.id)
                      markTouched('petType')
                    }}
                    className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all hand-drawn-border ${
                      data.petType === type.id
                        ? 'border-primary bg-primary/10'
                        : 'theme-border theme-card hover:theme-card-bg-hover'
                    }`}
                  >
              ...
              {touched.petType && errors?.petType && (
                <span className="text-red-500 text-xs mt-1 block">
                  {t?.validation?.petTypeRequired ?? 'Please select a pet type'}
                </span>
              )}
            </div>
            <div className="md:col-span-2">
              ...
              {touched.name && errors?.name && (
                <span className="text-red-500 text-xs mt-1 block">
                  {t?.validation?.petNameRequired ?? 'Required'}
                </span>
              )}
            </div>
            <div>
            ...
            <div>
              ...
              <FormInput
                ...
                min="0"
                max="30"
                value={typeof data.age === 'number' ? String(data.age) : (data.age ?? '')}
            ...
            <div>
              ...
              <FormInput
                ...
                max="200"
                step="0.1"
                value={typeof data.weight === 'number' ? String(data.weight) : (data.weight ?? '')}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (v === '') return
                  if (isGender(v)) updateData('gender', v)
                }}
              ...
              >
                ...
                <option value="m">{t?.labels?.male ?? 'Männlich'}</option>
                <option value="f">{t?.labels?.female ?? 'Weiblich'}</option>
              </select>
            </div>
          </div>
        </div>
