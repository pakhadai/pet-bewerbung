...
const Step3Description: React.FC<Step3DescriptionProps> = ({ onGenerate, embedded = false }) => {
  ...
  const safeSet = (text: string) => updateData('generatedText', text.slice(0, MAX_DESCRIPTION_LENGTH));
  ...
  const content = (
    <div className={`flex flex-col gap-6 ${embedded ? '' : 'p-8 lg:p-12 hand-drawn-border border-2 rounded-2xl'} ${embedded ? '' : cardCl} ${embedded ? '' : 'shadow-lg relative'}`}>
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-end flex-wrap gap-2">
              <label htmlFor="pet-description" className={`${embedded ? 'text-lg md:text-xl' : 'text-3xl'} font-display font-bold ${titleCl}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {templates.map((tpl) => (
                <button
                  key={tpl.id}
                  type="button"
                  onClick={() => ((data.generatedText || '').trim().length ? safeAppend(tpl.text) : safeSet(tpl.text))}
                  className={`text-sm font-semibold px-3 py-2 rounded-xl border transition-colors ${
                    darkMode ? 'border-gray-700 text-gray-100 hover:bg-gray-750' : 'border-gray-200 text-text-main hover:bg-gray-50'
                  }`}
              ...
              <button
                ...
                onClick={() => safeSet('')}
                className={`text-sm font-semibold px-3 py-2 rounded-xl border transition-colors ${
                  darkMode ? 'border-gray-700 text-gray-300 hover:bg-gray-750' : 'border-gray-200 text-text-secondary hover:bg-gray-50'
                }`}
                title={t?.builder?.clearTextHint ?? 'Clear the text field'}
              >
                <span className="inline-flex items-center gap-2">
                  <MaterialIcon name="backspace" className="text-lg" />
  ...
  return <div className={`page page-enter-${animDir} reveal fade-enter w-full max-w-2xl mx-auto pb-32`}>{content}</div>;
};
