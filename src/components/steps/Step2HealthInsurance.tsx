...
import type { BehaviorLevel, FormData, NoiseLevel } from '../../types/form'
...
const Step2HealthInsurance: React.FC<Step2HealthInsuranceProps> = ({ embedded = false }) => {
  ...
  const isNoiseLevel = (v: string): v is NoiseLevel => v === 'low' || v === 'medium' || v === 'high'
  const isBehaviorLevel = (v: string): v is BehaviorLevel =>
    v === 'good' || v === 'neutral' || v === 'avoid'
  ...
  const statusOptions = [
    ...
    {
      id: 'willingToPayDeposit',
      label: t?.labels?.willingToPayDeposit ?? 'Bereit für Tierkaution',
    },
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input
            value={data.vetPhone ?? ''}
            onChange={(e) => updateData('vetPhone', e.target.value)}
      ...
      <div className="mt-6 pt-6 border-t-2 border-dashed border-gray-400/50 dark:border-gray-500/50">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          >
            <span
              className={`pointer-events-none inline-block h-6 w-6 transform rounded-full bg-white shadow transition translate-y-0.5 ${showMore ? 'translate-x-7' : 'translate-x-1'}`}
            />
          </button>
        </div>
        ...
        {showMore && (
          <div className="mt-6 space-y-6 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="space-y-3">
              <h4 className={`font-display font-bold text-base ${titleCl}`}>
                {t?.labels?.insurance ?? 'Versicherung'}
              </h4>
              <Input
              ...
              />
              {INSURANCE_AFFILIATE_LINK &&
                (!data.insuranceProvider || String(data.insuranceProvider).length < 3) && (
                  <div
                    className={`mt-2 p-3 rounded-xl flex items-start gap-3 ${darkMode ? 'bg-blue-900/30 border border-blue-700/50' : 'bg-blue-50 border border-blue-200'}`}
                  >
                    <div
                      className={`p-2 rounded-full ${darkMode ? 'bg-blue-800 text-blue-300' : 'bg-white text-blue-600'}`}
                    >
                      <ShieldCheck size={20} />
                    </div>
                    <div className="flex-1">
                      <p
                        className={`text-sm font-bold ${darkMode ? 'text-blue-200' : 'text-blue-900'}`}
                      >
                      ...
                      <p className={`text-xs mt-1 ${darkMode ? 'text-blue-300' : 'text-blue-700'}`}>
                        {t?.affiliate?.insuranceDesc ??
                          'Vermieter verlangen oft einen Nachweis. Schützen Sie sich und Ihr Tier ab 5 CHF/Monat.'}
                      </p>
                      <a
                      ...
                      >
                        {t?.affiliate?.insuranceCta ?? 'Angebote vergleichen'}{' '}
              ...
              <div className="flex flex-col gap-2">
                {statusOptions.map((opt) => (
                  <label
                    key={opt.id}
                    className="flex items-center justify-between p-3 rounded-xl border-2 hand-drawn-border theme-card cursor-pointer hover:theme-card-bg-hover transition-colors"
                  >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    className="theme-input w-full p-3 border-2 hand-drawn-border rounded-xl text-sm"
                    value={data.noiseLevel ?? 'low'}
                    onChange={(e) => {
                      const v = e.target.value
                      if (isNoiseLevel(v)) updateData('noiseLevel', v)
                    }}
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const v = e.target.value
                      if (v === '') updateData('behaviorWithChildren', '')
                      else if (isBehaviorLevel(v)) updateData('behaviorWithChildren', v)
                    }}
                  >
                    <option value="">—</option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const v = e.target.value
                      if (v === '') updateData('behaviorWithPets', '')
                      else if (isBehaviorLevel(v)) updateData('behaviorWithPets', v)
                    }}
                  >
                    <option value="">—</option>
                    <option value="good">{t?.labels?.behaviorGood ?? 'Gut'}</option>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  updateData(
                    'medicalConditions',
                    e.target.value.slice(0, MAX_MEDICAL_CONDITIONS_LENGTH)
                  )
                }
