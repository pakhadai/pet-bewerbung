...
const FaqContent: React.FC<FaqContentProps> = ({ t, darkMode, className = '' }) => {
  ...
  return (
    <div className={`flex flex-col ${className}`}>
        ...
        {filteredQuestions.length > 0 ? (
          filteredQuestions.map((item) => (
              ...
              className={`border rounded-xl transition-all duration-300 overflow-hidden ${
                expandedId === item.id
                  ? darkMode
                    ...
                    : 'bg-blue-50/50 border-primary/30'
                  : darkMode
            ...
            >
              ...
              >
                <span
                  className={`font-bold pr-4 ${expandedId === item.id ? (darkMode ? 'text-primary' : 'text-primary-dark') : textMain}`}
                >
                  {item.q}
                </span>
                <span
                  className={`mt-1 transition-transform duration-300 flex-shrink-0 ${expandedId === item.id ? 'rotate-180 text-primary' : 'text-gray-400'}`}
                >
                  <ChevronDown size={20} />
                </span>
              </button>
          ...
          <div className="text-center py-10 opacity-50">
            ...
            <p>
              {faq?.noResults || 'Keine Ergebnisse'} &quot;{searchQuery}&quot;
            </p>
          </div>
        )}
      </div>
      ...
      {/* Footer Hint */}
      <div
        className={`p-4 border-t text-center text-xs flex-shrink-0 ${darkMode ? 'border-gray-800 bg-gray-900 text-gray-500' : 'border-gray-100 bg-gray-50 text-gray-400'}`}
      >
        {(() => {
          const hintText =
            faq?.footerHint || `Questions about the service? Email us at ${FAQ_SUPPORT_EMAIL}`
